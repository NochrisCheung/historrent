...
**status:** Phase 10 complete (2026-05-06) — loading screen + error overlay shipped. `uiStore` extended with `loadingStatus` (fonts → data → canvas → ready, partialised so it never persists). New components: `<LoadingOrchestrator>` (drives the sequence with `document.fonts.ready` + rAFs + 8 s timeout fallback), `<LoadingScreen>` (full-viewport overlay with thin progress bar + i18n status text), `<ErrorOverlay>` (centred card + retry → `window.location.reload()`), and `src/app/error.tsx` (Next.js boundary that renders the same overlay with the framework's `reset()` hook). `<LoadingFlow>` is the single mount point in `page.tsx`. 123 unit + 26 E2E green. Phase 11 (visual finalisation, user-driven) and Phase 12 (content curation) unblocked.
...
- **Fonts (current — Phase 6):** **`next/font/google`** for LXGW WenKai TC, Noto Serif SC (Hans content fallback — LXGW WenKai SC isn't on Google Fonts), Noto Sans SC, Noto Sans TC. CDN-served with `unicode-range`. **Deferred to Phase 11:** the original plan called for self-hosted `next/font/local` + manual `pyftsubset`; we'll revisit if (a) bundle/perf budgets demand a trim, or (b) Phase 11 visual review wants the Simplified LXGW WenKai (GB) variant which is only available outside Google Fonts.
...
| #   | Status  | Task                                                                                                                                                                                                                                                                                                                                                             |
| --- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2.1 | 🟢 DONE | **`src/data/liu_bang.schema.ts`** with the Zod schema in §3.3.                                                                                                                                                                                                                                                                                                   |
| 2.2 | 🟢 DONE | **`src/data/liu_bang.json`** — initial seed of **5 well-cited events** (suggested: birth ~256 BCE, 起兵 at 沛 209 BCE, 入咸阳 207 BCE, 鴻門宴 206 BCE, 即皇帝位 202 BCE). Each with both Hans/Hant name forms and ≥ 1 real ctext.org citation pointing into 高祖本紀 (chapter 8). Used to validate the schema and pipeline before the full curation in Phase 12. |
| 2.3 | 🟢 DONE | **`src/data/liu_bang.test.ts`** — `LiuBangCorpus.parse(...)` against the JSON. Failing schema = failing test.                                                                                                                                                                                                                                                    |
| 2.4 | 🟢 DONE | **`src/shared/date/bce.ts`** — BCE-aware year/month/day arithmetic, signed-year math, EDTF parsing for the small subset we use.                                                                                                                                                                                                                                  |
| 2.5 | 🟢 DONE | **`src/shared/date/bce.test.ts`** — unit tests for the math, including the year-0-doesn't-exist case (we use astronomical year numbering, where 1 BCE = year 0).                                                                                                                                                                                                 |
| 2.6 | 🟢 DONE | **`src/shared/citation/format.ts`** — render a `Citation` to a human-readable string in both languages.                                                                                                                                                                                                                                                          |
| 2.7 | 🟢 DONE | **`src/shared/citation/format.test.ts`** — snapshot tests.                                                                                                                                                                                                                                                                                                       |
| 2.8 | 🟢 DONE | **`src/components/canvas/geometry/yearToWorld.ts`** — `(year: number) => x: number`, mapping the lifespan to `[-5, +5]` with margin. Pure function, fully tested.                                                                                                                                                                                                |
...
| #    | Status  | Task                                                                                                                                                                                                                                                                                                   |
| ---- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 6.1  | 🟢 DONE | **`src/i18n/config.ts`** — next-intl config: list of locales `['zh-Hans', 'zh-Hant']`, default `zh-Hans`.                                                                                                                                                                                              |
| 6.2  | 🟢 DONE | **`src/i18n/messages/zh-Hans.json`** and **`zh-Hant.json`** — UI strings (loading status text, error retry, "show synthesis", "close", language-toggle labels, zoom toggle labels).                                                                                                                    |
| 6.3  | 🟢 DONE | **`src/i18n/Provider.tsx`** — client-side `<NextIntlClientProvider>` wrapper that reads the locale from `uiStore` and the corresponding messages catalog. Mounted in `app/layout.tsx`.                                                                                                                 |
| 6.4  | 🟢 DONE | **`src/state/uiStore.ts`** — Zustand store with `language: 'zh-Hans' \| 'zh-Hant'`, persisted to `sessionStorage`, default `zh-Hans`.                                                                                                                                                                  |
| 6.5  | 🟢 DONE | **`src/components/ui/LanguageToggle.tsx`** — small top-right control. Two-state segmented toggle: 簡 / 繁. Click updates `uiStore`.                                                                                                                                                                    |
| 6.6  | 🟢 DONE | Event display reads `event.name[language === 'zh-Hans' ? 'zhHans' : 'zhHant']`.                                                                                                                                                                                                                        |
| 6.7  | 🟢 DONE | **`scripts/subset-fonts.ts`** — Node script that calls `pyftsubset` (Python `fonttools`) to subset LXGW WenKai (Hans + Hant) and Source Han Sans to the union of (characters used in `liu_bang.json` + characters used in `messages/*.json` + ~3000 most-common Hanzi as a safety set). Outputs WOFF2. |
| 6.8  | 🟢 DONE | **`src/fonts/index.ts`** — `next/font/local` registrations pointing at the subsetted WOFF2 files in `public/fonts/`. Exposes `--font-content` (LXGW WenKai) and `--font-chrome` (Source Han Sans) CSS variables. `display: 'swap'`, preload primary face.                                              |
| 6.9  | 🟢 DONE | **Wire `next/font/local` into `app/layout.tsx`** — add the font CSS-variable className to the `<html>` element so tokens.css can reference them.                                                                                                                                                       |
| 6.10 | 🟢 DONE | ESLint custom rule `no-bare-strings-in-jsx` — flags hardcoded text in JSX, points to `useTranslations('key')`.                                                                                                                                                                                         |
| 6.11 | 🟢 DONE | **E2E `tests/e2e/i18n.spec.ts`** — toggle language; all visible text changes; reload-then-reopen-tab loses selection (sessionStorage); reload-same-tab preserves it.                                                                                                                                   |
...
### Phase 8.5 — Pan/zoom polish

**Trigger:** post-Phase-8 hand-test feedback (2026-05-05).

**Why:** Phase 8 shipped working controls but three rough edges turned up:

1. **Line and dots scale with the camera.** `STRING_HEIGHT = 0.02` and `BASE_RADIUS = 0.04` are in world units. At year zoom (`viewportWorldWidth = 12`) the line reads as ~2px tall and dots as ~4px — fine. At month zoom (`vw = 0.82`) the same world sizes balloon to ~31px-tall line and ~63px dots; at day zoom they're absurd. Pixel sizes need to stay invariant.
2. **Drag has no peripheral motion cue.** Currently `uCurveCenter = cameraX` so the entire curve pattern (envelope + wave) is camera-locked: as the user drags, the events translate but the string itself looks identical. Visually it can feel like nothing's moving.
3. **No quantitative scale readout.** The user can wheel-zoom to any free-form `viewportWorldWidth`, but there's no map-style scale bar telling them what they're looking at — "one year on screen" vs "one decade on screen."

**Reference geometry:** `WORLD_PER_YEAR = 2 × TIMELINE_WORLD_HALF_WIDTH / LIU_BANG_LIFESPAN = 10/61 ≈ 0.164`. Used by the legend to convert world units ↔ years.

| #      | Status  | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 8.5.1  | 🟢 DONE | **Pixel-stable line + dots.** Define `viewportScale = viewportWorldWidth / GRANULARITY_WIDTHS.year`. In `<TimelineString>`, apply `scale={[1, viewportScale, 1]}` so the mesh's y dimension shrinks proportionally to viewport — line stays ~2px tall at every zoom. In `<TimelineItem>`, restructure into `<group position={[x, y, 0.01]}><mesh>…</mesh><Html>…</Html><Html>…</Html></group>` so the inner dot mesh can be uniformly scaled (combining hover-spring scale with `viewportScale` via two refs that the spring's `onChange` and a `viewportScale`-effect both read) without dragging the `<Html>` labels along; scale the `<Html>` y-offsets by `viewportScale` so the label-to-dot pixel distance stays constant. The hover spring still drives a 1.0 → 1.5 multiplier; viewport changes apply through the same `setScalar(hoverRef × viewportRef)` path so the two animation sources compose cleanly without a stale-closure race.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| 8.5.2  | 🟢 DONE | **World-anchored wave phase.** In `shaders.ts` change the curl wave argument from `curveWave(xRel)` to `curveWave(position.x)`. In `geometry/curve.ts#curveYAt`, change `curveWave(xRel)` to `curveWave(x)` (the world-x parameter). Envelope (gating where the flat zone is) stays camera-relative — the held-flat centre still tracks the camera. Wave _phase_ now ties to world position, so as the user drags, the high-frequency wobble in the curl tails appears to scroll laterally. The overall drop silhouette stays anchored to the camera (envelope is unchanged), so the editorial "always-flat at viewport centre" framing is preserved.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
| 8.5.3  | 🟢 DONE | **`src/components/ui/IntervalLegend.tsx` + `.module.css`.** Bottom-left fixed overlay (`position: fixed; bottom: var(--s-3); left: var(--s-3)`). A short horizontal bar with a label below. Bar pixel width derived each render from `viewportWorldWidth` and `granularity`: pick `(intervalYears, label)` per granularity — year → `(10, "10 年")`, month → `(1, "1 年")`, day → `(1/12, "1 月")`. Pixel width = `intervalYears × WORLD_PER_YEAR / viewportWorldWidth × window.innerWidth`. Subscribes to `cameraStore` so the bar width animates smoothly during wheel zoom and during the spring snap. Aria-label is i18n'd under `intervalLegend.ariaLabel` (Hans + Hant); the unit characters 年/月 are universal across both scripts.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 8.5.4  | 🟢 DONE | **`geometry/curve.test.ts`** — extend with a test that pins the new world-anchored-wave behaviour: with default uniforms (wobble on), `curveYAt(0 + xRel, {uCurveCenter: 0})` differs from `curveYAt(5 + xRel, {uCurveCenter: 5})` for an `xRel` past the flat zone, because the wave argument now uses world-x rather than xRel. Existing wobble-disabled symmetry tests remain valid because they multiply the wave by 0.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 8.5.5  | 🟢 DONE | **`tests/e2e/zoom.spec.ts`** — add an assertion that `[data-testid="interval-legend"]` exists and renders the expected label per granularity ("10 年" → "1 年" after clicking month). Bar width is exercised implicitly through the granularity assertion (label and width are computed from the same source).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 8.5.6  | 🟢 DONE | Verify lint, typecheck, all unit tests, build, full E2E suite — all green.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 8.5.7  | 🟢 DONE | **Always-on event labels (user revision, 2026-05-05).** Phase 4 hid labels behind hover to de-cluster the year-zoom cluster; user prefers visibility over crowding. Drop the hover-gated opacity on both `<Html>` overlays in `<TimelineItem>`; labels render permanently. **Known tradeoff:** four events at the right cluster (uprising → death, +3 to +5 world-x) overlap their name labels at year zoom. Date labels are shorter and overlap less. Re-clutter (significance filter / vertical stagger / collision avoidance) is left as a follow-on iteration; this task only flips the default visibility.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 8.5.8  | 🟢 DONE | **IntervalLegend matches selected granularity (user revision, 2026-05-05).** Phase 8.5.3 picked `10 年 / 1 年 / 1 月` (one-step-bigger pattern). User wants the bar to read as one unit of the toggle's current selection so it lines up with the visible UI. Update `LEGEND_BY_GRANULARITY`: year → `1 × WORLD_PER_YEAR` / `1 年`; month → `(1/12) × WORLD_PER_YEAR` / `1 月`; day → `(1/365.25) × WORLD_PER_YEAR` / `1 日`. Pixel widths become small (~15–25 px on a 1366-wide viewport), but the legend is now semantically locked to the toggle. Update the E2E expectations to match.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 8.5.9  | 🟢 DONE | **Decouple camera zoom from string (user revision #2, 2026-05-05).** User wants the string's visual appearance (curl shape, wobble pattern, flat-zone proportions) to stay identical across year/month/day, with events instead spreading radially from `cameraX` as the granularity tightens. Architectural pivot: camera zoom stops tracking `viewportWorldWidth` and becomes constant at `canvas_width / GRANULARITY_WIDTHS.year`; `<TimelineItem>` now positions each event at `cameraX + (originalX − cameraX) × eventScale` where `eventScale = GRANULARITY_WIDTHS.year / viewportWorldWidth` (year=1, month≈14.6, day≈292.7). Screen position of every event evaluates identically in both models, so drag / wheel / cursor-anchoring math is unchanged. Phase 8.5.1's pixel-stable compensation becomes redundant (camera no longer zooms → no compensation needed); revert it on `<TimelineString>` and `<TimelineItem>` and update the now-stale doc comments. The curve's y for an event is computed at its rendered (stretched) world-x — `curveYAt(renderedX, …)` — so events near `cameraX` sit on the flat zone and events far from it ride the curl tails.                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 8.5.10 | 🟢 DONE | **Per-event label lane placement (user revision #1, 2026-05-05).** Built on top of 8.5.9 because lane assignment uses the rendered screen-x. New helper `src/components/canvas/labelPlacement.ts#computeLabelLanes(events, cameraX, viewportWorldWidth, canvasWidthPx)` returns `{[eventId]: laneIndex}`. Algorithm: compute each event's stretched world-x, then screen-x; sort by screen-x; walk in order — for each event, find the lowest lane whose most-recent occupant is more than `LABEL_OVERLAP_THRESHOLD_PX` (~90 px) away and place it there. `<TimelineItem>` accepts a `lane` prop and offsets each `<Html>` y-position by `lane × LANE_STEP_PX` (~22 px); when `lane > 0` an inline 1 px vertical leader line connects the label back to the dot so the user can see which name belongs to which event. `<Timeline>` subscribes to `cameraX` + `viewportWorldWidth` + canvas size, calls `computeLabelLanes` once per render, and passes the lane index per event. Unit tests: clustered events get incrementing lanes; widely-spaced events all get lane 0. E2E: pan to centre the right cluster, assert no two `[data-event-name]` bounding rects intersect.                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 8.5.11 | 🟢 DONE | **Straight string + viewport-relative edge blur (user revision #5, 2026-05-05).** User no longer wants the curl/wave; the string should read as a clean horizontal line with the visible left/right ends fading to transparent. Vertex shader stops displacing y entirely (drops envelope/curl/wave math, drops the four curve-shape uniforms); a new `vWorldX` varying just passes `position.x` through. Fragment shader's alpha falloff switches from mesh-relative (`vUv.x`) to viewport-relative — `t = \|vWorldX − uCurveCenter\| / uViewportHalfWidth`, with `uViewportHalfWidth = GRANULARITY_WIDTHS.year / 2 = 6` (constant since 8.5.9 fixed the camera zoom). Default falloff start drops from `0.7` to `0.5` so the fade has runway across the visible viewport. **Cleanup:** `geometry/curve.ts` — drop `curveYAt`, `curveWave`, `smoothstep`; `CurveUniforms` shrinks to `{ uCurveCenter, uAlphaFalloffStart, uAlphaFalloffEnd, uViewportHalfWidth }`. `useCurveStore` keeps only the two alpha-falloff knobs (with `uViewportHalfWidth` derived as a constant). `<CurveTuner>` simplifies to two sliders. `<TimelineItem>` no longer subscribes to curve-shape uniforms; `y = 0`. `curve.test.ts` is deleted (its three subjects all removed). **Note:** Phase 8.5.2's world-anchored wave doc value is now moot — wave is gone; the rationale stays in the plan as history but no longer applies to running code.                                                                                                                                                                                |
| 8.5.12 | 🟢 DONE | **Lane placement direction flip (user revision #6, 2026-05-05).** User screenshot showed the right-cluster getting `0, 1, 2, 0` (rising staircase peaking at the third event 鸿门宴, then plummeting to 0 for 即皇帝位). They want the third event nearer the line. **Root cause:** `computeLabelLanes` walks events left-to-right; the rightmost member of a tight cluster always ends up on the highest lane just before lane 0 recycles. **Fix:** flip the sort order to right-to-left. Same algorithm, same min-coloring (3 lanes are unavoidable for 3 pairwise-conflicting events), just choose the assignment that puts lane 2 in the middle rather than at the right end of the cluster. For Liu Bang's right cluster the result becomes `0, 2, 1, 0` — an arch with the peak at 攻入咸阳, descending into 鸿门宴 (lane 1) and then 即皇帝位 (lane 0). **Tests:** three existing labelPlacement tests need updated expectations (assignments invert within each tight cluster); E2E "no overlap" assertion is unchanged because it doesn't pin specific lanes.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
| 8.5.13 | 🟢 DONE | **Cluster-based lane placement (user revision #7, 2026-05-05).** The right-to-left fix from 8.5.12 worked at the right end of the cluster but introduced a symmetric problem at the left: 立为沛公 (the cluster's leftmost) was bumped to lane 2 because xianyang's right-to-left walk stole lane 0 from accession's neighbourhood. The user now asks for **both ends close to the line** — an arch with the peak in the middle. Both pure-direction greedy walks produce a one-sided staircase; the only way to get an arch is to anchor BOTH cluster ends at lane 0 first. **Fix:** detect clusters by adjacent-gap (a cluster is a maximal run of events whose neighbour gap is `< LABEL_OVERLAP_THRESHOLD_PX`); within each cluster, place the leftmost event at lane 0, then process the remaining cluster events right-to-left. Across clusters, lane 0 is shared globally where pairwise distance allows. For Liu Bang's right cluster the result is `0, 2, 1, 0` reading left-to-right: 立为沛公 at lane 0, 攻入咸阳 (peak) at lane 2, 鸿门宴 at lane 1, 即皇帝位 at lane 0. **Limitation:** for a 5-event clique (all pairwise conflicting) the algorithm still degrades to a descending staircase since 5 mutually-conflicting labels can't fit a balanced arch — but Liu Bang's data never reaches 5-clique depth at typical zoom. **Tests:** two labelPlacement test expectations flip back to the cluster-symmetric layout (`tightly-packed three` and `two clusters separated`); the 4-event recycle test happens to match both algorithms and stays unchanged; E2E `no-overlap` assertion stays. |

**Acceptance:** At month/day zoom the timeline string and event dots stay readable (no fat blobs); the labels stay pixel-pinned to their dots. The string is a clean horizontal line whose left/right ends fade to transparent across the outer half of the viewport. Event names + dates are always visible AND don't overlap (lanes auto-stagger crowded clusters). Events spread radially from `cameraX` as the user toggles to finer granularities. The interval legend tracks the selected granularity (`1 年` / `1 月` / `1 日`).

### Phase 9 — AI source synthesis (route handler + UI)

**Strategy decisions locked 2026-05-06:**

- **Per-call shape.** One DeepSeek call = one event in one language. The route is `POST /api/synthesise { eventId, language }`. Switching language triggers a separate call with its own cache entry. Most events are never visited; we don't pay for them.
- **Prompt input is curated, not bulk.** A single call's user prompt contains: event name + date + a handful of pre-curated Shiji paragraph excerpts (1–5 per event). NOT the whole Shiji, NOT the whole chapter. Each excerpt prefixed with `[Shiji-{chapter}-{paragraph}]`. The model cites with the same bracketed labels in its answer.
- **Output shape.** ~80–120 chars Chinese, plain text with inline `[Shiji-N-M]` citation references. No retries; refusal text is rendered as the answer (see 9.12).
- **Schema option A** (chosen): the passage text lives **inline on each citation entry** in `liu_bang.json` (rather than a separate `passages` map). A normalised model — each citation is self-contained — and `prompt.ts` derives the bracketed label from the citation's `chapter + paragraph` fields.
- **UUIDs option B** (chosen): each event gains a stable `id: uuid` field; the existing slug becomes `slug: string`. The cache key uses `id` (UUID, rename-stable). DOM data attributes and E2E selectors switch to `slug` (still human-readable). React keys / Zustand store switch to `id`.
- **Build order:** server-deepest-first. Schema migration (9.0) → prompt (9.3, 9.4) → cache (9.5, 9.6) → deepseek (9.7) → handler (9.1, 9.8) → route (9.2) → client (9.9) → UI (9.10–9.12) → guards + E2E (9.13, 9.14). Every piece is unit-testable in isolation before the next one is wired on.
- **Dev-time DeepSeek:** stubbed by default. The DI seam in `handleSynthesise()` selects a `FakeDeepSeekClient` returning a deterministic synthesis when `DEEPSEEK_API_KEY` is unset; the real fetch-based client only runs once the env var is set. Mirrors the cache's Upstash/in-memory branch.
- **Network shape:** non-streaming (`stream: false`) standard `chat/completions` request. (DeepSeek's "batch mode" — async 24 h jobs at 50 % off — is a different API and not used.) Response time at our prompt size on `deepseek-v4-flash` is ~500 ms–2 s.
- **No rate limit / cost control in v1.** Cache forever; bump `promptVersion` to invalidate.

| #    | Status  | Task                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| ---- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 9.0a | 🟢 DONE | **Schema migration — UUIDs + slug + inline passage text.** In `src/data/liu_bang.schema.ts`: add `id: z.string().uuid()` (event UUID); rename existing `id` field (slug) to `slug: z.string()`; add `text: z.string()` to each `citation` entry alongside the existing `chapter`, `paragraph`, `anchor`, `url`. Generate UUID v4 for each of the 5 existing events (`crypto.randomUUID()` once, paste into JSON, never change). Add seed `text` per citation — placeholder lifted from ctext.org for now; Phase 12 will hand-verify. Update `liu_bang.test.ts` to validate the new shape.                                                                  |
| 9.0b | 🟢 DONE | **Plumb the rename through the rest of the codebase.** `<TimelineItem>`, `<TimelineString>`, `<DetailPanel>`, `useTimelineStore`, `labelPlacement` etc. — switch React keys + store interactions to `event.id` (UUID), switch DOM `data-event-name` / `data-event-date` to `event.slug` (so the rendered DOM attributes stay readable: `data-event-name="hongmen-banquet"`). E2E shared helpers (`hoverDot`, `panCameraToOrigin`, etc.) keep using slug strings. Run lint, typecheck, full unit + E2E gates.                                                                                                                                               |
| 9.3  | 🟢 DONE | **`src/ai/server/prompt.ts`** — pure function `buildSynthesisPrompt(event, language) → { system, user, version }`. Builds the multi-paragraph user message: `事件：{name}（约 {date}）` + each `citation` as `[Shiji-{chapter}-{paragraph}] {citation.text}` joined by blank lines + the closing instruction line. System prompt per §3.7 (verbatim, version `"v1"`). Computes `passageHash = sha256(citation.text concatenated in citation order)` and exposes it on the return value so the handler can include it in the cache key. Returns the same prompt for both Hans and Hant inputs at the source level — only the request-language line differs. |
| 9.4  | 🟢 DONE | **`src/ai/server/prompt.test.ts`** — snapshot tests against fixture event payloads (5 events × 2 languages = 10 snapshots). Verifies: bracketed labels match `Shiji-{chapter}-{paragraph}`, citations appear in their `liu_bang.json` order, system prompt is byte-stable across languages, `passageHash` is deterministic, `version` is the literal `"v1"`.                                                                                                                                                                                                                                                                                               |
| 9.5  | 🟢 DONE | **`src/ai/server/cache.ts`** — exports `getCachedSynthesis(key)` and `setCachedSynthesis(key, value)`. Backend selection: if `UPSTASH_REDIS_REST_URL` + `_TOKEN` set → `@upstash/redis`; else process-scoped `Map`. Key = `synthesis-v1:${sha256(model + promptVersion + event.id + language + passageHash)}` — uses **event.id (UUID)**, not slug, so renaming the human-facing slug never invalidates a cache. TTL = none; bump `promptVersion` (in `prompt.ts`) to invalidate everything. Value = `{ synthesis: string, generatedAt: string, model: string, promptVersion: string }`.                                                                   |
| 9.6  | 🟢 DONE | **`src/ai/server/cache.test.ts`** — verifies the in-memory backend behaves identically to the Upstash interface: get-after-set returns value; get on missing returns undefined; same key idempotent; key shape includes the `synthesis-v1:` prefix.                                                                                                                                                                                                                                                                                                                                                                                                        |
| 9.7  | 🟢 DONE | **`src/ai/server/deepseek.ts`** — fetch-based client. Reads `DEEPSEEK_API_KEY` from `process.env`. POSTs to `https://api.deepseek.com/chat/completions` with `{ model: "deepseek-v4-flash", messages: [system, user], stream: false }`. Throws typed errors on rate limit (`DeepSeekRateLimitError`), network (`DeepSeekNetworkError`), non-200 (`DeepSeekHttpError`). Exports `createDeepSeekClient()` which the handler calls — returns an object `{ chat(prompt) → Promise<{content: string}> }`. Sibling export `createFakeDeepSeekClient(fixture?)` returns a deterministic synthesis for dev/tests.                                                  |
| 9.1  | 🟢 DONE | **`src/ai/server/handler.ts`** — pure function `handleSynthesise({ eventId, language, deps }) → Promise<SynthesisResponse>` where `eventId` is the **event UUID**. Validates inputs with Zod, loads the event from `src/data/liu_bang.json` by id, composes: cache lookup → if miss, prompt build → DeepSeek call → cache write → response. Returns `{ synthesis, eventId, language, citations, cached, promptVersion, generatedAt }`. Deps (`cache`, `deepseek`) are injected; the route picks real or fake based on env. **Refusal flows through verbatim** — handler does no detection or retry on the synthesis text.                                  |
| 9.8  | 🟢 DONE | **`src/ai/server/handler.test.ts`** — unit-tests the pure handler with stubbed `cache` and `deepseek`. Verifies: cache hit returns immediately without calling DeepSeek; cache miss calls DeepSeek then writes cache; refusal text flows through unchanged; bumping `promptVersion` invalidates a cache hit; switching language produces a different cache key; switching the event's UUID produces a different cache key.                                                                                                                                                                                                                                 |
| 9.2  | 🟢 DONE | **`src/app/api/synthesise/route.ts`** — thin route file: parses request body, calls `handleSynthesise()` with deps wired from env (`createDeepSeekClient` if `DEEPSEEK_API_KEY` set, else `createFakeDeepSeekClient`; `createCache` follows the same pattern), maps result to `Response`. `export const runtime = 'nodejs'`. ~30 lines.                                                                                                                                                                                                                                                                                                                    |
| 9.9  | 🟢 DONE | **`src/ai/client/client.ts`** — frontend caller. POSTs to `/api/synthesise` with `fetch`, handles loading + error states. Returns `{ synthesis, citations, cached, ... }` or throws a typed error. Has no knowledge of DeepSeek; just talks to our route.                                                                                                                                                                                                                                                                                                                                                                                                  |
| 9.10 | 🟢 DONE | **DetailPanel "show synthesis" button** — calls `client.synthesise(event.id, language)`. Loading dot animation (existing `--accent` motion token) while pending. Error: inline retry button (`text-button`, "重试"). After success the button collapses and the synthesis renders in its place.                                                                                                                                                                                                                                                                                                                                                            |
| 9.11 | 🟢 DONE | **Synthesis renders below citations**, with `[Shiji-N-M]` references rendered as small clickable chips. Each chip's `onClick` scrolls the matching citation card (already rendered above) into view via `scrollIntoView({ behavior: "smooth", block: "center" })`. Citation cards gain `id="citation-Shiji-{chapter}-{paragraph}"` for direct anchor matching. Chip parsing happens client-side: regex splits the synthesis text on `\[Shiji-\d+-\d+\]` boundaries.                                                                                                                                                                                        |
| 9.12 | 🟢 DONE | **Refusal-as-answer rendering.** Handler treats refusal text identically to a normal synthesis. UI does the same — no special heuristic. The user sees the model's verbatim "passages do not contain enough information" line as the rendered synthesis. Snapshot one example refusal in `handler.test.ts` to lock the contract.                                                                                                                                                                                                                                                                                                                           |
| 9.13 | 🟢 DONE | **ESLint guard** — `no-restricted-imports` rule preventing client-side files (anything outside `src/app/api/**` or `**/*.test.ts`) from importing `@/ai/server/*`. Already configured in Phase 0.18; extend with the new server paths.                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 9.14 | 🟢 DONE | **E2E `tests/e2e/ai.spec.ts`** with `/api/synthesise` mocked via Playwright route interception. Tests: cache miss → loading → result with rendered chips; cache hit → instant result (no loading state); 500 error → retry button → success after retry. Also: clicking a chip scrolls its citation card into the visible area.                                                                                                                                                                                                                                                                                                                            |

**Acceptance:** Click an event → "show synthesis" → loading → ~1 s later, a cited synthesis appears with bracketed labels rendered as chips that scroll to citation cards. Reload + click same event → synthesis appears instantly (cache hit). Network failure → inline retry button. Costs negligible (≤ $0.02 per full corpus exploration). Local dev with no API key returns a deterministic stub.

### Phase 10 — Loading screen and error overlay

| #    | Status  | Task                                                                                                                                                                                                                                                                                                                   |
| ---- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 10.1 | 🟢 DONE | **`src/components/ui/LoadingScreen.tsx`** — full-viewport overlay until `READY`. Centred elegant progress bar (thin, animated, `--accent` filling on `--surface` track). Status text below, internationalised: "加载历史…", "加载字体…", "加载数据…".                                                                  |
| 10.2 | 🟢 DONE | **Asset preloading orchestrator** in `src/app/page.tsx` — sequence: load fonts (next/font handles bulk; we wait via `document.fonts.ready`), parse data through Zod, warm up canvas (one render pass). Each step updates loading-status state in `uiStore`.                                                            |
| 10.3 | 🟢 DONE | **`src/components/ui/ErrorOverlay.tsx`** — centred message on white-overlay layer (full viewport, `--canvas-bg` at 95% opacity). Retry button. Used for: data parse failure, font load failure, AI failure (when persistent). Wired via `src/app/error.tsx` for unhandled errors and via local state for handled ones. |
| 10.4 | 🟢 DONE | **E2E** — simulate a font load failure (route block) → error overlay appears with retry; retry route allow → loads cleanly.                                                                                                                                                                                            |
...
| 6.6  | **Font loading**                  | 🟢 **`next/font/google` for Phase 6; `next/font/local` + `pyftsubset` deferred to Phase 11**                            | 2026-05-05 | Phase 6 ships LXGW WenKai TC, Noto Serif SC, Noto Sans SC, Noto Sans TC via Google Fonts CDN with `unicode-range`. Self-hosted SC LXGW WenKai (GB variant; not on Google Fonts) and pyftsubset trimming revisited in Phase 11 if bundle budgets or visual fidelity demand it. |
