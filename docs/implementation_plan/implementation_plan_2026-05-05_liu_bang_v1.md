...
**status:** Phase 8.5 complete (2026-05-05) — pan/zoom polish + two user revisions: pixel-stable thickness, world-anchored wave phase, always-on event labels, IntervalLegend reads as one unit of the selected toggle (1 年 / 1 月 / 1 日). 102 unit + 20 E2E green. Phase 9 (AI synthesis) unblocked.
...
**Acceptance:** At month/day zoom the timeline string and event dots stay readable (no fat blobs); the labels stay pixel-pinned to their dots. Dragging the canvas at year zoom shows the wobble pattern in the curl tails scrolling horizontally as a peripheral motion cue. Event names + dates are always visible (overlap at year zoom is acknowledged and deferred). The interval legend tracks the selected granularity (`1 年` / `1 月` / `1 日`) with its bar width recomputed from `viewportWorldWidth` each render. **Outcome:** all five feedback items resolved across the three commits.
