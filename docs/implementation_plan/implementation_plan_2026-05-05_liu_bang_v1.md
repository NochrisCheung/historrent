...
**status:** Phase 8.5 complete (2026-05-05) — pan/zoom polish + four user revisions. Camera zoom decoupled from granularity (string visual is now invariant; events spread radially from cameraX as the user toggles to finer granularities); per-event lane stagger with leader lines so always-on labels in dense clusters don't overlap. 109 unit + 21 E2E green. Phase 9 (AI synthesis) unblocked.
...
**Acceptance:** At month/day zoom the timeline string and event dots stay readable (no fat blobs); the labels stay pixel-pinned to their dots. Dragging the canvas shows the wobble pattern in the curl tails scrolling horizontally as a peripheral motion cue. Event names + dates are always visible AND don't overlap (lanes auto-stagger crowded clusters). The string's visual appearance is invariant across granularities; events spread radially from `cameraX` as the user toggles to finer granularities. The interval legend tracks the selected granularity (`1 年` / `1 月` / `1 日`).
