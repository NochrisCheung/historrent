...
**status:** Phase 9 complete (2026-05-06) — AI source synthesis end-to-end. Schema migrated to UUID `id` + readable `slug` + per-citation `text`. Server pipeline: `prompt.ts` (with passageHash) → `cache.ts` (Upstash + Map fallback) → `deepseek.ts` (real fetch + deterministic fake) → `handler.ts` → `/api/synthesise` route. Frontend: `client.ts` + `<DetailPanel>` synthesis section with idle/loading/error/ready states + `[Shiji-N-M]` clickable chips that scrollIntoView the citation card. 120 unit + 24 E2E green. Phase 10 (loading screen) unblocked.
